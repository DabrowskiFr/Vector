...
Module VectorTheory (Import P : Process) (Import V : Vector P).
  ...
  Hint Resolve π_prop1 π_prop2 vzeq : vector.
  ...
  Proof.
    ...
    - exists (vempty _ H__Eq); intuition omega.
    - assert (∃ (f : nat -> A), ∀ i, i < S n -> P i (f i)) as [f Hf].
      {
        assert (S n <= p) by omega; clear H__Eq.
        induction n.
        - assert (0 < p) as H__lt by omega.
          destruct (H__all 0 H__lt) as [a Ha].
          exists (fun x => a).
          intros i H__lt2.
          now replace i with 0 by omega.
        - assert (S n <= p) as H__le by omega.
          destruct (IHn H__le) as [f Hb].
          assert (S n < p) as H__lt by omega.
      ...
      intros i H__lt.
      exists (f i).
      rewrite π_make by omega; intuition.
  Qed.
  ...
  Proof.
    intros A B P H__all.
    case_eq p; [intros H__Eq | intros n H__Eq].
    - exists (vempty _ H__Eq), (vempty _ H__Eq) ; intuition omega.
  ...
  Hint Resolve vectmap_eq vectmap_some_rev vectmap_none_rev.
                        ...
                        forall i, i < k -> π i (fmap f v') = π i v).
    {
      ...
      - assert (k <= p) as HV by intuition.
        ...
        + assert (i <> k) as HW by omega.
          ...
          rewrite Hx.
          intuition.
        + assert (i = k) as HW  by omega; subst.
  ...
  Hint Resolve vectmap_some_rev vectmap_none_rev : vectmap.
  ...
  Hint Resolve vectmap_extensionality vectmap_compose : vectmap.
